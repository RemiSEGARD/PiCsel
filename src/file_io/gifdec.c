...
gd_open_gif(const char *fname)
{
    ...
    /* Read GCT */
    gif->gct.size = gct_sz;
    r = read(fd, gif->gct.colors, 3 * gif->gct.size);
    gif->palette = &gif->gct;
    gif->bgindex = bgidx;
...
read_graphic_control_ext(gd_GIF *gif)
{
    ...
    int r = read(gif->fd, &rdit, 1);
    (void) r;
    gif->gce.disposal = (rdit >> 2) & 3;
    gif->gce.input = rdit & 2;
    gif->gce.transparency = rdit & 1;
    gif->gce.delay = read_num(gif->fd);
    r = read(gif->fd, &gif->gce.tindex, 1);
...
read_application_ext(gd_GIF *gif)
{
    ...
    int r = read(gif->fd, app_id, 8);
    (void) r;
    /* Application Authentication Code. */
    r = read(gif->fd, app_auth_code, 3);
...
get_key(gd_GIF *gif, int key_size, uint8_t *sub_len, uint8_t *shift, uint8_t *byte)
{
    ...
    for (bits_read = 0; bits_read < key_size; bits_read += frag_size) {
        ...
        if (rpad == 0) {
            /* Update byte. */
            if (*sub_len == 0) {
                r = read(gif->fd, sub_len, 1); /* Must be nonzero! */
                if (*sub_len == 0)
                    return 0x1000;
            }
            r = read(gif->fd, byte, 1);
            (*sub_len)--;
        }
...
read_image_data(gd_GIF *gif, int interlace)
{
    ...
    free(table);
    if (key == stop)
        r = read(gif->fd, &sub_len, 1); /* Must be zero! */
...
read_image(gd_GIF *gif)
{
    ...
    if (fisrz & 0x80) {
        ...
        r = read(gif->fd, gif->lct.colors, 3 * gif->lct.size);
        gif->palette = &gif->lct;
    } else
        gif->palette = &gif->gct;
    /* Image Data. */
...
Frame *import_gif(char *filename)
{
    SDL_Surface *surface;
    gd_GIF *gif;
    Uint8 *color, *frame;
    ...
    Frame *f = init_frame(gif->width, gif->height);
    Frame *tmpf = f->next;
    while (1) {
        ret = gd_get_frame(gif);
        surface = tmpf->layer->next->img;
        if (ret == -1)
            break;
        ...
        for (i = 0; i < gif->height; i++) {
            for (j = 0; j < gif->width; j++) {
                if (!gd_is_bgcolor(gif, color))
                    pixel = SDL_MapRGBA(surface->format, color[0], color[1], color[2], 255);
                else 
                    pixel = SDL_MapRGBA(surface->format, 0, 0, 0, 0); //mettre alpha 0
                addr = surface->pixels + (i * surface->pitch + j * sizeof(pixel));
                memcpy(addr, &pixel, sizeof(pixel));
                color += 3;
            }
        }
        tmpf->duration = gif->gce.delay * 10;
        if (ret == 0)
            break;
        else
        {
            add_frame(f, gif->width, gif->height, 1);
            tmpf = tmpf->next;
        }
    }
    free(frame);
